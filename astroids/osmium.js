...
TODO -- replace enemyship by asteroid
     -- add explosion on collision
     -- randomly place pointsBuilding
     -- limit no. of bullets - add remaining bullets HUD
     -- ad  guided missile - player can get these by colliding with pointsBuilding
...
function GameMaster(width, height) {
    ...
    this.lastPointsBoxSpawnedTime = 0;
    ...
    this.init = function() {
        ...
        this.GUILayer.addFrame(new GuiFrame(70, 555, 100, 40, "grey-panel", "Bullets: ", "bullets", this.player));
        this.GUILayer.addFrame(new GuiFrame(200, 555, 100, 40, "grey-panel", "Lives: ", "lives", this.player));
        this.GUILayer.addFrame(new GuiFrame(480, 555, 100, 40, "grey-panel", "Score: ", "score", this.player));
    ...
    this.newObject = function(type, parent, x, y) {
        if (type == 'bullet') {
            ...
            if (this.meteorPool.length) {
                obj = this.meteorPool.pop();
                obj.reinit(x, y);
                obj.addToScene();                
            } else {
                this.addObject(new Meteor(x, y));
            }
        } else if (type == 'points_box') {
            if (this.pointsBoxPool.length) {
                obj = this.pointsBoxPool.pop();
                obj.reinit(x, y);
                obj.addToScene();                
            } else {
                this.addObject(new PointsBox(x, y));
            }
        }
    };
    ...
    this.gameLoop = function(timestamp) {
        ...
        if (timestamp - self.lastPointsBoxSpawnedTime > (self.difficulty) * 50) {
            self.spawnPointsBox();
            self.lastPointsBoxSpawnedTime = timestamp;
        }
    ...
    this.spawnEnemy = function() {
        ...
        var y = 50;
        this.newObject('meteor', null, x, y);
    };

    this.spawnPointsBox = function() {
        var x = Math.random() * (this.params.width - 200);
        var y = 50;
        this.newObject('points_box', null, x, y);
    };
...
function PlayerShip(x, y) {
    ...
    this.fireBullet = function() {
        ...
        if (now - this.lastFired > this.firingRate && this.bullets > 0) {
            this.lastFired = now;
    ...
    this.onCollide = function(obj) {
        ...
        if (obj.type == 'meteor') {
            ...
            if(this.immunity && ((window.performance.now() - this.immunityInitTime) > 3000)) 
                    this.immunity = false; 
        }
    };
    ...
    this.repair = function() {
        if(this.lives >= 0){
            //restart and provide immunity for 3 seconds
            this.immunity = true;
            this.immunityInitTime = window.performance.now();
        }
        else{
...
function Meteor(x, y) {
    ...
    this.obj = new window.game.engine.Image(document.getElementById('meteor'), {
        left: x,
        top: y,
        originX: 'center',
        originY: 'center',
    ...
    this.onCollide = function(obj) {
        if(obj.type == 'player'){
            if(this.active)window.game.addToPool(this);
            this.health = 0;
            this.explode();
        }
        if(obj.type == 'bullet'){
...
function PointsBox(x, y) {
    this.type = "points_box";
    this.isFriendly = false;
    ...
    this.obj = new window.game.engine.Image(document.getElementById('pointsBuilding'), {
        left: x,
        top: y,
        originX: 'center',
        originY: 'center',
    ...
    this.onCollide = function(obj) {
        if(obj.type == "player" && this.active){
            window.game.addToPool(this);
            window.game.player.bullets += 5;
        }
    };
};


function Explosion(parentObject) {
    this.type = "explosion";
    ...
    this.width = 50;
    this.height = 50;
    this.obj = new window.game.engine.Image(document.getElementById('explosion'), {
        left: this.x,
        top: this.y,
        originX: 'center',
        originY: 'center',
    ...
    this.add = function() {
        ...
        window.game.canvas.add(this.obj);
        window.setTimeout(function(){self.remove();},2000)
    }
...
function FragMissile(parentObject) {
    this.parent = parentObject;
